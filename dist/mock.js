...
const h = (pw) => bcryptjs_1.default.hashSync(pw, 8);
let users = [
...
function auth(req, res, next) {
    ...
    if (!header || !header.startsWith('Bearer '))
        return fail(res, 401, 'Not authorized, token missing.');
    try {
...
const sign = (user) => jsonwebtoken_1.default.sign({ id: user.id, role: user.role }, JWT_SECRET, { expiresIn: '7d' });
...
app.post('/api/v1/auth/register', async (req, res) => {
    ...
    const user = {
        ...
        passwordHash: bcryptjs_1.default.hashSync(password, 8),
        role: 'customer',
...
app.post('/api/v1/auth/login', async (req, res) => {
    ...
    if (!user || !bcryptjs_1.default.compareSync(String(password ?? ''), user.passwordHash)) {
...
app.get('/api/v1/books', (req, res) => {
    ...
    if (search && String(search).trim()) {
        ...
        list = list.filter((b) => b.title.toLowerCase().includes(term) || b.author.toLowerCase().includes(term) || b.isbn.toLowerCase().includes(term));
    }
    const sorters = {
...
app.post('/api/v1/cart', auth, (req, res) => {
    ...
    if (!book)
        return fail(res, 404, 'Book not found.');
    if (book.stock <= 0)
        return fail(res, 400, 'This book is currently out of stock.');
    const existing = cartItems.find((i) => i.user === req.user.id && i.book === book._id);
    if (existing)
        existing.quantity = Math.min(existing.quantity + Number(req.body?.quantity ?? 1), book.stock);
    else
        cartItems.push({ _id: `ci-${++seq}`, user: req.user.id, book: book._id, quantity: Number(req.body?.quantity ?? 1) });
...
// ---------------------------------------------------------------- wishlist
const wishFor = (userId) => wishlistItems
    .filter((i) => i.user === userId)
    .map((i) => ({ ...i, book: populateBook(bookById(i.book)) }));
...
app.post('/api/v1/wishlist', auth, (req, res) => {
    ...
    if (existing)
        return ok(res, { ...existing, book: populateBook(book) });
...
app.post('/api/v1/orders', auth, (req, res) => {
    ...
    for (const item of cart.items) {
        ...
        if (item.quantity > b.stock)
            return fail(res, 400, `Not enough stock for "${b.title}".`);
    }
    for (const item of cart.items)
        bookById(item.book._id).stock -= item.quantity;
    cartItems = cartItems.filter((i) => i.user !== req.user.id);
    const order = {
        _id: `o-${++seq}`,
...
app.post('/api/v1/admin/books', requireAdminMw, (req, res) => {
    ...
    if (!catById(category))
        return fail(res, 400, 'Please choose a valid category.');
    const book = {
        _id: `b-${++seq}`,
        title: title.trim(),
        author: author.trim(),
...
app.use((err, _req, res, _next) => {
    const status = err instanceof multer_1.default.MulterError || String(err?.message ?? '').includes('Only JPEG') ? 400 : 500;
